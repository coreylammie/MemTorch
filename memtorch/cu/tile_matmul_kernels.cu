...
__global__ void tile_matmul_kernel(
  ...
  if (i < limit_i && j < limit_j && k < limit_k) {
    ...
    Eigen::MatrixXf tile_b = Eigen::Map<Eigen::MatrixXf, Eigen::RowMajor,
        ...
        Eigen::Stride<1, Eigen::Dynamic>(1, mat_b_tiles_shape[2]));
    Eigen::VectorXf partial_sum = (tile_a * tile_b).transpose();
    float *partial_sum_data =
        (float *)malloc(sizeof(float) * partial_sum.size());
    memcpy(partial_sum_data, partial_sum.data(),
           sizeof(float) * partial_sum.size());
    float sf = det_sf_(partial_sum_data, partial_sum.size(), ADC_resolution,
                       overflow_rate, NULL, NULL);
    free(partial_sum_data);
    partial_sum =
        linear_quantize(partial_sum, sf, ADC_resolution, overflow_rate);
#pragma omp parallel for
    for (int ii = 0; ii < partial_sum.size(); ii++) {
      result[transform_2d_index(i, j * mat_b_tiles_shape[2] + ii,
                                mat_b_shape_back)] += partial_sum[ii];
