...
__device__ float quantize_element(float element, float *quant_levels,
...
__global__ void quantize(int num_quant_levels, float *quant_levels,
                         int num_elements, float *tensor) {
...
__global__ void quantize_(int num_quant_levels, float *min_values,
                          float *max_values, int num_elements, float *tensor) {
...
void quant_cuda(at::Tensor tensor, int num_quant_levels, float min_value,
  ...
  torch::Tensor quant_levels =
      at::linspace(min_value, max_value, num_quant_levels);
  float *quant_levels_gpu;
  cudaMalloc(&quant_levels_gpu, sizeof(float) * quant_levels.numel());
  cudaMemcpy(quant_levels_gpu, quant_levels.data<float>(),
             sizeof(float) * quant_levels.numel(), cudaMemcpyHostToDevice);
  quantize<<<GET_BLOCKS(tensor.numel()), CUDA_NUM_THREADS, 0,
             at::cuda::getCurrentCUDAStream()>>>(
      num_quant_levels, quant_levels_gpu, tensor.numel(), tensor.data<float>());
...
void quant_cuda(at::Tensor tensor, int num_quant_levels, at::Tensor min_values,
  ...
  quantize_<<<GET_BLOCKS(tensor.numel()), CUDA_NUM_THREADS, 0,
              at::cuda::getCurrentCUDAStream()>>>(
      num_quant_levels, min_values.data<float>(), max_values.data<float>(),
      tensor.numel(), tensor.data<float>());
  cudaDeviceSynchronize();
  cudaStreamSynchronize(at::cuda::getCurrentCUDAStream());
}
