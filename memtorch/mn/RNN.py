...
class RNN(nn.RNN):
	...
	def __init__(
        ...
        **kwargs
		...
		assert isinstance(
			rnn_layer, nn.RNN
		), "rnn_layer is not an instance of nn.RNN."
		...
		for i in range(rnn_layer.num_layers):
			...
			if rnn_layer.bidirectional:
				self.w_ih_reverse.append(rnn_layer._parameters['weight_ih_l{i}_reverse'].data)
				self.w_ih_reverse[i].requires_grad = False
				self.w_hh_reverse.append(rnn_layer._parameters['weight_hh_l{i}_reverse'].data)
				self.w_hh_reverse[i].requires_grad = False
				if rnn_layer.bias:
					self.b_ih_reverse.append(rnn_layer._parameters['bias_ih_l{i}_reverse'].data)
					self.b_ih_reverse[i].requires_grad = False
					self.b_hh_reverse.append(rnn_layer._parameters['bias_hh_l{i}_reverse'].data)
					self.b_hh_reverse[i].requires_grad = False

		warnings.warn("RNN layers are not fully supported. Only legacy forward passes are currently enabled.")
	...
	def forward(self, input, h_0=None):
		...
		for layer in range(self.num_layers):
			...
			for t in range(inp.shape[0]):	
				if self.bias:
					h_t = torch.matmul(inp[t], self.w_ih[layer].T) + self.b_ih[layer] + torch.matmul(h_t, self.w_hh[layer].T) + self.b_hh[layer]
				else:
					h_t = torch.matmul(inp[t], self.w_ih[layer].T) + torch.matmul(h_t, self.w_hh[layer].T)
			...
			if self.bidirectional:
				...
				for t in range(inp.shape[0]):
					if self.bias:
						h_t_reverse = torch.matmul(inp[-1 - t], self.w_ih_reverse[layer].T) + self.b_ih_reverse[layer] + torch.matmul(h_t_reverse, self.w_hh_reverse[layer].T) + self.b_hh_reverse[layer]
					else:
						h_t_reverse = torch.matmul(inp[-1 - t], self.w_ih_reverse[layer].T) + torch.matmul(h_t_reverse, self.w_hh_reverse[layer].T)
