...
cs *cs_spalloc (csi m, csi n, csi nzmax, csi values, csi triplet)
{
    ...
    A->p = (ptrdiff_t *)cs_malloc (triplet ? nzmax : n+1, sizeof (csi)) ;
    A->i = (ptrdiff_t *)cs_malloc (nzmax, sizeof (csi)) ;
...
csi cs_sprealloc (cs *A, csi nzmax)
{
    ...
    if (CS_TRIPLET (A)) A->p = (ptrdiff_t *)cs_realloc (A->p, nzmax, sizeof (csi), &okj) ;
...
csd *cs_dalloc (csi m, csi n)
{
    ...
    if (!D) return (NULL) ;
    D->p = (ptrdiff_t *)cs_malloc (m, sizeof (csi)) ;
    D->r = (ptrdiff_t *)cs_malloc (m+6, sizeof (csi)) ;
    D->q = (ptrdiff_t *)cs_malloc (n, sizeof (csi)) ;
    D->s = (ptrdiff_t *)cs_malloc (n+6, sizeof (csi)) ;
