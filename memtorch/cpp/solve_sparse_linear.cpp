...
void solve_sparse_linear_bindings(py::module_ &m) {
  m.def("solve_sparse_linear", [](at::Tensor A_indices_x,
    ...
    float *A_indices_x_accessor = A_indices_x.data_ptr<float>();
    float *A_indices_y_accessor = A_indices_y.data_ptr<float>();
    float *A_values_accessor = A_values.data_ptr<float>();
    float *B_accessor = B.data_ptr<float>();
    int A_nonzero_elements = A_values.sizes()[0];
    Eigen::Map<Eigen::VectorXf> A_indices_x_vector(&A_indices_x_accessor[0],
                                                   A_nonzero_elements);
    Eigen::Map<Eigen::VectorXf> A_indices_y_vector(&A_indices_y_accessor[0],
                                                   A_nonzero_elements);
    Eigen::Map<Eigen::VectorXf> A_values_vector(&A_values_accessor[0],
                                                A_nonzero_elements);
    Eigen::Map<Eigen::VectorXf> B_vector(&B_accessor[0], B.sizes()[0]);
    ...
    memcpy(X_tensor.data_ptr<float>(), X.data(), sizeof(float) * X.size());
    return X_tensor;
  });
}